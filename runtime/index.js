...
entrypoint: {
  ...
  if (!command && inputFile && !Prefs.e && !Prefs.p) {
    if (Prefs.target === 'c' || Prefs.target === 'native') {
      command = Prefs.target;
      ...
      if (nonOptOutFile) {
        ...
        tmpRunDir = fs.mkdtempSync(path.join(os.tmpdir(), 'porffor-run-'));
        Prefs.o = path.join(tmpRunDir, process.platform === 'win32' ? 'out.exe' : 'out');
        Prefs.quiet = true;
        runAfterCompile = !Prefs.noRun;
      }
    }
  }
