...
export default ({ funcs, data = [], globals = [], entry = null, prefs = {}, usedTypes = null, zigvm = null }) => {
  const out = [];
  ...
  const zigvmCType = type => ({ i32: 'i32', f64: 'f64', void: 'void', pointer: 'void*', buffer: 'void*' })[type];
  const zigvmExports = [];
  const zigvmSafepoints = [];
  const zigvmHostImports = zigvm?.hostImports ?? [];
  if (zigvmEnabled) {
    if (!Number.isInteger(zigvmAbiVersion) || zigvmAbiVersion < 1)
      throw new Error(`invalid --zigvm-abi-version: ${prefs.zigvmAbiVersion}`);

    for (const f of funcs) {
      if (!f?.export || f.name === entry) continue;
      const name = f.exportName ?? f.name;
      const params = f.zigvmAbiParamTypes ?? [];
      const result = f.zigvmAbiReturnType;
      if (params.length !== (f.ast?.params?.length ?? params.length) || params.some(x => !zigvmCType(x) || x === 'void'))
        throw new Error(`zigvm export ${name} requires i32/f64 TypeScript annotations on all parameters`);
      if (!result || !['i32', 'f64', 'void'].includes(result))
        throw new Error(`zigvm export ${name} requires an i32/f64/void TypeScript return annotation`);
      zigvmExports.push({ f, name, symbol: sanitize(name), params, result });
      zigvmSafepoints.push({ function: name, kind: 'entry', flag: 1, export: sanitize(name) });
    }

    let opcodeIndex = 0;
    const scan = (node, funcName) => {
      ...
      if (isNode(node)) {
        ...
        opcodeIndex++;
        scan(node[N_A], funcName); scan(node[N_B], funcName); scan(node[N_C], funcName);
        return;
      }
      for (const child of node) scan(child, funcName);
    };
    for (const f of funcs) if (f?.body) scan(f.body, f.exportName ?? f.name);
  }
  ...
  const renderExpr = node => {
    switch (node[N_KIND]) {
      ...
      case K.CallDynamic: {
        ...
        if (spreadArr) {
          const call = `porf_call_dynamic_arr(${rx(node[N_A], P_COMMA)}, ${rx(node[N_B], P_COMMA)}, ${newt}, ${rx(spreadArr, P_COMMA)})`;
          return [zigvmEnabled ? `(zvm_porf_safepoint(ZVM_PORF_SAFEPOINT_CALL), ${call})` : call, P_POSTFIX];
        }
        const argv = args.length === 0 ? '(jsbits[]){JV_UNDEFINED_BITS}'
          : `(jsbits[]){ ${args.map(packArg).join(', ')} }`;
        const call = `porf_call_dynamic(${rx(node[N_A], P_COMMA)}, ${rx(node[N_B], P_COMMA)}, ${newt}, ${args.length}, ${argv})`;
        return [zigvmEnabled ? `(zvm_porf_safepoint(ZVM_PORF_SAFEPOINT_CALL), ${call})` : call, P_POSTFIX];
      }
  ...
  if (zigvmEnabled) {
    const hostTypedefs = [];
    const hostFields = [];
    for (const host of zigvmHostImports) {
      const result = zigvmCType(host.result);
      const params = host.parameters.map((type, i) => {
        const cType = zigvmCType(type);
        if (!cType || type === 'void') throw new Error(`unsupported zigvm host parameter type for ${host.name}.${i}: ${type}`);
        return `${cType} a${i}`;
      });
      if (!result) throw new Error(`unsupported zigvm host return type for ${host.name}: ${host.result}`);
      const name = sanitize(host.name);
      hostTypedefs.push(`typedef ${result} (*zvm_porf_host_${name}_fn)(void* ctx${params.length ? ', ' + params.join(', ') : ''});`);
      hostFields.push(`  zvm_porf_host_${name}_fn ${name};`);
    }
    head.push(`
#if defined(_WIN32)
#define ZVM_PORF_EXPORT __declspec(dllexport)
#else
#define ZVM_PORF_EXPORT __attribute__((visibility("default")))
#endif
#define ZVM_PORF_ABI_VERSION ${zigvmAbiVersion}u
#define ZVM_PORF_SAFEPOINT_ENTRY 0x1u
#define ZVM_PORF_SAFEPOINT_BACKEDGE 0x2u
#define ZVM_PORF_SAFEPOINT_CALL 0x4u
#define ZVM_PORF_SAFEPOINT_ASYNC 0x8u
#define ZVM_PORF_SAFEPOINT_UNWIND 0x10u
typedef struct zvm_porf_runtime_api {
  u32 abi_version;
  void* ctx;
  void (*safepoint)(void* ctx, u32 flags);
  u32 (*poll_reload)(void* ctx, u32 flags);
  void (*trap)(void* ctx, u32 code);
} zvm_porf_runtime_api_t;
${hostTypedefs.join('\n')}
typedef struct zvm_porf_host_api {
  u32 abi_version;
  void* ctx;
${hostFields.join('\n')}
} zvm_porf_host_api_t;
static zvm_porf_runtime_api_t zvm_porf_runtime_api_storage;
static const zvm_porf_runtime_api_t* zvm_porf_runtime_api = NULL;
static zvm_porf_host_api_t zvm_porf_host_api_storage;
static const zvm_porf_host_api_t* zvm_porf_host_api = NULL;
static inline void zvm_porf_safepoint(u32 flags) {
  zvm_porf_runtime_api->safepoint(zvm_porf_runtime_api->ctx, flags);
}
`);
  }
...
`);
  ...
  if (zigvmEnabled) {
    const main = funcByName.get(entry);
    if (!main) throw new Error('missing zigvm module entry function');
    const wrappers = [];
    wrappers.push('static i32 zvm_porf_initialized = 0;');
    wrappers.push('ZVM_PORF_EXPORT i32 zvm_porf_init(const zvm_porf_runtime_api_t* api) {');
    wrappers.push('  if (api == NULL || api->abi_version != ZVM_PORF_ABI_VERSION) return -1;');
    wrappers.push('  if (api->safepoint == NULL || api->poll_reload == NULL || api->trap == NULL) return -1;');
    wrappers.push('  zvm_porf_runtime_api_storage = *api;');
    wrappers.push('  zvm_porf_runtime_api = &zvm_porf_runtime_api_storage;');
    wrappers.push('  if (!zvm_porf_initialized) {');
    wrappers.push('    porf_init(0, NULL);');
    wrappers.push('    porf_data_init();');
    if (gcEnabled) {
      wrappers.push('    volatile int porf_stack_anchor = 0;');
      wrappers.push('    porf_c_stack_top = (void*)&porf_stack_anchor;');
    }
    wrappers.push(`    ${fnSym(main)}();`);
    wrappers.push('    zvm_porf_initialized = 1;');
    wrappers.push('  }');
    wrappers.push('  return 0;');
    wrappers.push('}');
    wrappers.push('');
    wrappers.push('ZVM_PORF_EXPORT i32 zvm_porf_set_host_api(const zvm_porf_host_api_t* api) {');
    if (zigvmHostImports.length > 0) {
      wrappers.push('  if (api == NULL || api->abi_version != ZVM_PORF_ABI_VERSION) return -1;');
      for (const host of zigvmHostImports) wrappers.push(`  if (api->${sanitize(host.name)} == NULL) return -1;`);
      wrappers.push('  zvm_porf_host_api_storage = *api;');
      wrappers.push('  zvm_porf_host_api = &zvm_porf_host_api_storage;');
    } else {
      wrappers.push('  (void)api;');
    }
    wrappers.push('  return 0;');
    wrappers.push('}');
    wrappers.push('');
    ...
    for (const exported of zigvmExports) {
      const cParams = exported.params.map((type, i) => `${zigvmCType(type)} a${i}`);
      const callArgs = [];
      let userArg = 0;
      for (const param of exported.f.params) {
        if (param.name === '#env') callArgs.push('0');
        else if (param.name[0] === '#') {
          if (param.name === '#this' || param.name === '#newtarget' || param.name === '#callee') callArgs.push('JV_UNDEFINED');
          else throw new Error(`unsupported hidden parameter ${param.name} on zigvm export ${exported.name}`);
        } else callArgs.push(`a${userArg++}`);
      }
      const call = `${fnSym(exported.f)}(${callArgs.join(', ')})`;
      wrappers.push(`ZVM_PORF_EXPORT ${zigvmCType(exported.result)} ${exported.symbol}(${cParams.join(', ') || 'void'}) {`);
      if (gcEnabled) {
        wrappers.push('  volatile int porf_stack_anchor = 0;');
        wrappers.push('  porf_c_stack_top = (void*)&porf_stack_anchor;');
      }
      wrappers.push('  zvm_porf_safepoint(ZVM_PORF_SAFEPOINT_ENTRY);');
      if (exported.result === 'void') wrappers.push(`  ${call};`);
      else wrappers.push(`  return ${call};`);
      wrappers.push('}');
      wrappers.push('');
    }
    emit(wrappers.join('\n'));
  ...
  if (entry && !prefs.nativeFetch && !zigvmEnabled) {
  ...
  if (zigvmEnabled) {
    return {
      ...
      zigvmManifest: {
        version: 1,
        module: globalThis.file ?? 'module',
        exports: zigvmExports.map(x => ({ name: x.name, symbol: x.symbol, params: x.params, result: x.result })),
        host_imports: zigvmHostImports.map(x => ({ name: x.name, parameters: x.parameters, result: x.result })),
        runtime_api: {
          name: 'zvm_porf_runtime_api_t',
          abi_version: zigvmAbiVersion,
          flags: { entry: 1, backedge: 2, call: 4, async: 8, unwind: 16 }
        },
        safepoints: zigvmSafepoints,
...
#if defined(_WIN32)
  if (VirtualAlloc(MEM + porf_heap_committed, want - porf_heap_committed,
...
#if defined(_WIN32)
  void* got = VirtualAlloc(NULL, (SIZE_T)PORF_ARENA_RESERVE, MEM_RESERVE, PAGE_NOACCESS);
  if (got == NULL) {
#else
...
#if defined(_WIN32)
static LARGE_INTEGER porf_performance_start;
static LARGE_INTEGER porf_performance_frequency;
#else
...
static inline void porf_performance_init(void) {
  if (!porf_performance_started) {
    ...
    QueryPerformanceFrequency(&porf_performance_frequency);
    QueryPerformanceCounter(&porf_performance_start);
    porf_performance_time_origin_value = (f64)(ticks.QuadPart - 116444736000000000ull) / 10000.0;
#else
...
${sti}f64 porf_performance_now(void) {
  ...
  return (f64)(now.QuadPart - porf_performance_start.QuadPart) * 1000.0 /
    (f64)porf_performance_frequency.QuadPart;
#else
  struct timespec now;
