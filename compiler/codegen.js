...
const generateCall = (scope, decl) => {
  ...
  if (zigvmHostImport) {
    if (decl._new || decl.arguments.some(x => x?.type === 'SpreadElement'))
      throw new Error(`zigvm host import ${name} does not support constructor or spread calls`);
    if (decl.arguments.length !== zigvmHostImport.parameters.length)
      throw new Error(`zigvm host import ${name} expects ${zigvmHostImport.parameters.length} arguments, got ${decl.arguments.length}`);
    const args = decl.arguments.map((arg, i) =>
...
const extractTypeAnnotation = decl => {
  ...
  let types = null, type = null, typeName = null, elementType = null, irType = null;
  if (a.typeName) {
  ...
  return { type, types, typeName, elementType, irType };
};
...
const zigvmAbiIrType = (type, label, allowVoid = false) => {
  switch (type) {
    case 'i32': return T.i32;
    case 'f64': return T.f64;
    case 'pointer':
    case 'buffer': return T.ptr;
    case 'void': if (allowVoid) return T.none;
  }
  throw new Error(`unsupported zigvm ABI type for ${label}: ${type}`);
};
...
const registerZigvmHostImports = (pattern, init) => {
  if (!Prefs.zigvm) throw new Error('Porffor.dlopen is not yet supported in the native IR backend');
  ...
  for (const binding of pattern.properties) {
    ...
    const spec = objectProperty(schema, sourceName);
    if (!sourceName || !localName || spec?.type !== 'ObjectExpression')
      throw new Error('invalid zigvm host import declaration');

    const parameterNode = objectProperty(spec, 'parameters');
    const resultNode = objectProperty(spec, 'result');
    const parameters = parameterNode?.type === 'ArrayExpression'
      ? parameterNode.elements.map(x => x?.value)
      : [];
    const result = resultNode?.value ?? 'void';
    ...
    zigvmHostImports.set(localName, { name: sourceName, parameters, result });
  }
};
...
const generateVarDstr = (scope, kind, pattern, init, defaultValue, global) => {
  if (init && init.type === 'CallExpression' && init.callee.name === '__Porffor_dlopen') {
    registerZigvmHostImports(pattern, init);
    return valUndefined();
  }
...
const generateFunc = (scope, decl, forceNoExpr = false) => {
  ...
  if (typedInput && decl.returnType) {
    const { type, types, typeName, irType } = extractTypeAnnotation(decl.returnType);
    func.zigvmAbiReturnType = typeName;
    if (typeName === 'void') func.retType = T.none;
    else if (irType != null) func.retType = irType;
  ...
  for (let i = 0; i < params.length; i++) {
    ...
    const abiType = typedInput && typeAnnotation ? extractTypeAnnotation(typeAnnotation).typeName : null;
    args.push({ name: argName, def, destr, type: typedInput && typeAnnotation, zigvmAbiType: abiType,
...
export default (program, opts = {}) => {
  ...
  return {
    ...
    usedTypes,
    zigvm: Prefs.zigvm ? { hostImports: [ ...zigvmHostImports.values() ] } : null
  };
};
